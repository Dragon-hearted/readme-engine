 ...
 */
export function renderPlatformSvg(
	systems: SystemData[],
	options?: { hubTagline?: string },
): string {
	...
	if (count === 0) {
		return svgDoc(
			...
			200,
			"",
			`  <text x="${cx}" y="100" text-anchor="middle" fill="${DARK.secondary}" font-family="${FONTS.body}" font-size="14">No systems registered</text>`,
		);
	}
	...
	const defs = [glowFilterDefs(allColors), arrowMarkerDef(DARK.secondary, "hub-arrow")].join("\n");
